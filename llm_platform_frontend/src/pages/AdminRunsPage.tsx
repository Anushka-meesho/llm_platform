...
const AdminRunsPage = () => {
  // Filters persist, so the history view returns with the same search/page/
  // filters after a reload.
  ...
  // The history is a point-in-time snapshot, NOT a live view. anchorRef holds the
  // run id the list is sliced against. The first load after a fresh page load
  // leaves it null, so the server pins the snapshot to the newest run at that
  // moment and returns its id, which we stash here. Every page change and filter
  // edit then reuses this same anchor, so a run inserted while you browse never
  // shifts the pages under you — page 2 stays the rows that were on page 2 when
  // you started. Pressing Refresh clears it to take a new snapshot at "now".
  ...
  const load = useCallback(() => {
    setLoading(true);
    api
      .adminRuns({ ...filters, anchorId: anchorRef.current ?? undefined })
      .then((d) => {
        // First load of a snapshot: remember the anchor the server pinned to, so
        // every later page slices against this same point in time.
        if (anchorRef.current == null) anchorRef.current = d.anchor_id;
        setData(d);
        setError(null);
        setForbidden(false);
      })
  ...
  // Debounced fetch: every filter/page change (incl. typing) settles for 300ms
  ...
  // Refresh takes a NEW snapshot: drop the anchor so the next load re-pins to the
  // newest run, then reload the current page/filters against that fresh point.
  ...
  return (
    <div className="flex-1 overflow-y-auto bg-primary-bg p-6">
      <div className="mx-auto max-w-6xl flex flex-col gap-5">
        ...
        {/* Pagination — above the table so the page controls are reachable
            without scrolling past a full page of rows. */}
          ...
          <div className="bg-secondary-bg px-4 py-2.5 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-3">
              <Typography variant="body" size="1" className="text-tertiary-text">
                Snapshot view — newest at refresh
              </Typography>
              <Button
                variant="outline"
                size="s"
                onClick={refresh}
                disabled={loading}
                title="Take a fresh snapshot of the latest runs"
              >
                ↻ Refresh
              </Button>
            </div>
...
// GoToPage is a "jump straight to page N" box for the paginator: type a number,
// press Enter or Go, and the list jumps there. Out-of-range / empty input is
// ignored. The parent remounts it on every page change (key={page}) so the
// placeholder always reflects the page you're actually on.
const GoToPage = ({
  page,
  totalPages,
  onGo,
  ...
  const submit = () => {
    ...
    if (!Number.isNaN(n) && n >= 1 && n <= totalPages && n !== page) onGo(n);
    setVal('');
  };
  return (
    <div className="flex items-center gap-1.5">
      <span className="text-xs text-tertiary-text">Go to</span>
      <input
        type="number"
        min={1}
        max={totalPages}
      ...
      />
      <Button variant="outline" size="s" onClick={submit} disabled={!val}>
        Go
      </Button>
    </div>
  );
