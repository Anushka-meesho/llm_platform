...
import type { TLeaderboardEntry, TModel } from '../types';
                  ...
                  <div className="flex items-center justify-between mb-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Typography variant="body" size="3" className="text-primary-text font-semi-bold">
                        {MODEL_LABELS[e.model as TModel] ?? e.model}
                      </Typography>
                    </div>
            ...
            {top && (
              <div className="border-t border-solid border-primary-border pt-3">
                <Typography variant="body" size="2" className="text-tertiary-text">
                  {tied.length > 1
                    ? `${tied.map((e) => MODEL_LABELS[e.model as TModel] ?? e.model).join(' and ')} are tied at ${top.avg_score.toFixed(1)}/5.`
                    : `"${MODEL_LABELS[top.model as TModel] ?? top.model}" leads with an average of ${top.avg_score.toFixed(1)}/5.`}{' '}
