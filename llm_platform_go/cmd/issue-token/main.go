...
func main() {
	...
	role := flag.String("role", auth.DefaultRole, "RBAC role: admin|creator|approver|caller|viewer")
	...
	if !auth.KnownRole(*role) {
		log.Fatalf("unknown role %q (want admin|creator|approver|caller|viewer)", *role)
	}
