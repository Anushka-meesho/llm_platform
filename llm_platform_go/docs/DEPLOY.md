# Deploying the LLM Platform — `llm_platform_go`
...
> When standing up the companion frontends in their own repos, **don't** carry this
> repo's local `llm_platform.db` or `.env` into them — they are dev-only artifacts and
...
### 5.3 The multi-stage Dockerfile (API only)

A production `Dockerfile` ships at the repo root — build it directly with
`docker build -t llm-platform .` (or `make docker-build`). It is a multi-stage,
cgo-free build that produces a static binary on a non-root distroless base:
...
RUN go mod download
COPY . .
RUN CGO_ENABLED=0 go build -trimpath -ldflags="-s -w" -o /out/llm-platform ./cmd/server \
...
COPY --from=build /out/llm-platform /llm-platform
...
before rolling the new server, since the prod server does not auto-migrate; `/bootstrap`
is the once-per-environment first-run tool. A `.dockerignore` keeps secrets and the
local DB out of the build context.
