...
| What if it's down? | A **fallback chain** + **circuit breaker** automatically routes around failures. |
...
```mermaid
graph TD
    PB[Product Builder<br/>writes tasks + prompts] -->|REST API| BE[Go Backend]
    AP[Approver<br/>reviews + deploys prompts] -->|REST API| BE
    SA[Service / App<br/>calls predict endpoint] -->|REST API| BE
...
- **Approver** — the human gate before a prompt goes live. They can deploy versions but can't write prompts.
...
The task defines the signature (inputs/outputs) and the implementation (which model, what prompt). Your service code only calls the function — it never sees the implementation.
...
> ⚠️ **Tradeoff:** You lose access to provider-specific features that aren't in the OpenAI format — e.g., Anthropic's "thinking" mode or Gemini's native multimodal document API. When those features are needed, you'd add a native client for that provider and give it its own registry entry.
...
sequenceDiagram
    ...
    API->>API: Validate JWT, check permission
    API->>API: Load task config (from memory cache)
    API->>API: Validate inputs against schema
    API->>API: Render prompt template with inputs
    API->>Cache: Is this exact prediction cached?
    Cache-->>API: No (miss)
    API->>Model: Call GPT-4o with rendered prompt
    Model-->>API: {"label": "shipping", "confidence": 0.94}
    API->>API: Validate output against output schema
    API->>Cache: Store result (24h TTL)
    API-->>App: 200 OK {"output": {...}, "cost_usd": 0.0002}
    API->>API: Log run to DB (async — doesn't block response)
```
